...
public class AddReview implements ICommand {
    ...
    private void diagnoseError(HttpServletRequest request) {
        if (SessionElements.getLocale(request).equals("ru")) {
            request.setAttribute(AttributeName.ADD_REVIEW_ERROR.getValue(), "Пожалуйста, поставьте оценку!");
        } else {
            request.setAttribute(AttributeName.ADD_REVIEW_ERROR.getValue(), "You hadn't choose anything to add");
        }
    }
}
