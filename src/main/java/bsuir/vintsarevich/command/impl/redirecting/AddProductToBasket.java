...
public class AddProductToBasket implements ICommand {
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            clientId = ((User) request.getSession().getAttribute(AttributeName.USER.getValue())).getId();
            ...
            for (Product product : allProducts) {
                if (request.getParameter(AttributeName.PRODUCT_ID.getValue() + "_" + product.getId()) != null &&
                        request.getParameter(AttributeName.NUMBER_FOR_ADD.getValue() + "_" + product.getId()) != null) {

                    productId = Integer.valueOf(request.getParameter(AttributeName.PRODUCT_ID.getValue() + "_" + product.getId()));
                    productCount = Integer.valueOf(request.getParameter(AttributeName.NUMBER_FOR_ADD.getValue() + "_" + product.getId()));
    ...
    private void diagnoseError(HttpServletRequest request) {
        if (SessionElements.getLocale(request).equals("ru")) {
            request.setAttribute(AttributeName.ADD_PRODUCT_TO_BASKET_ERROR.getValue(), "Ничего не выбрано");
        } else {
            request.setAttribute(AttributeName.ADD_PRODUCT_TO_BASKET_ERROR.getValue(), "You hadn't choose anything to add");
        }
    }
}